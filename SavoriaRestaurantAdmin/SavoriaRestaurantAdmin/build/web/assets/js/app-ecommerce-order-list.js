...
$(function () {
	...
	n.length && (t = n.DataTable({
		ajax: "assets/json/ecommerce-customer-order.json",
		columns: [{
			data: "id"
		}, {
			data: "id"
		...
		columnDefs: [{
			...
			render: function (t, e, a, s) {
				var n = a.customer,
					r = a.email,
					o = a.avatar;
				return '<div class="d-flex justify-content-start align-items-center order-name text-nowrap"  data-email="' + r + '"><div class="avatar-wrapper"><div class="avatar me-2">' + (o ? '<img src="' + assetsPath + "img/avatars/" + o + '" alt="Avatar" class="rounded-circle">' : '<span class="avatar-initial rounded-circle bg-label-' + ["success", "danger", "warning", "info", "dark", "primary", "secondary"][Math.floor(6 * Math.random())] + '">' + (o = (((o = (n = a.customer).match(/\b\w/g) || []).shift() || "") + (o.pop() || "")).toUpperCase()) + "</span>") + '</div></div><div class="d-flex flex-column"><h6 class="m-0"><a href="pages-profile-user.html" class="text-body">' + n + '</a></h6><small class="text-muted" name="customer_email">' + r + "</small></div></div>"
			}
		}, {
			targets: 5,
			render: function (t, e, a, s) {
...
document.addEventListener('DOMContentLoaded', function () {
	var orderLinks = document.querySelectorAll('a[href="/GetOrder"]');

	orderLinks.forEach(function (link) {
		link.addEventListener('click', function (event) {
			event.preventDefault();
			var email = this.closest('.order-name').dataset.email;
			var url = this.href + '?customerEmail=' + encodeURIComponent(email);
